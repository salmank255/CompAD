...
Dataset_name = 'videos'
output_path = 'Activitynet_vis_results'
...
for file_path in glob.glob(Dataset_name+'/*.mp4'):
    ...
    fig = plt.figure(figsize=(12.8*2,9.6*2))
    ...
    for f_no in range(len(rois)):
        ...
        ax.set_zlabel('Y')
        plt.show()
        plt.savefig('foo_act.png')
        graph_img_ = cv2.imread('foo_act.png')
        graph_img = graph_img_[430:1425,1005:2080:]
        graph_img = cv2.resize(graph_img,(inp_img.shape[1],inp_img.shape[0]))
