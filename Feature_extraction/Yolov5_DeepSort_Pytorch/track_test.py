...
def detect(opt,inp_data):
    ...
    # for source in glob.glob('../../waymo_clips/val/*.mp4'):
        source = 'train_00296.mp4'
        vid_id_set = set()
        tube_ids = {}
        # print(source)
        # with open(source+'.json','r') as fff:
        #     inpJson = json.load(fff)
        ...
        for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
            ...
            # frame_anno = inpJson["frames"][str(frame_idx+1)]
            # n_boxes = len(frame_anno)
            print(img.shape)
            print(im0s.shape)
            # print(frame_idx)
                ...
                if det is not None and len(det):
                    ...
                    if len(outputs) > 0:
                        for j, (output, conf) in enumerate(zip(outputs, confs)): 
                            ...
                            # print('box_id',id)
                            if id not in vid_id_set:
                                vid_id_set.add(id)
                                tube_id = get_random_alphanumeric_string(32)
                                tube_ids[id] = tube_id
                            ...
                            inpJson["frames"][str(frame_idx+1)].append({"x1": b_x1,
                                                                        "y1": b_y1,
                                                                        "x2": b_x2,
                                                                        "y2": b_y2,
                                                                        "width":1920,
                                                                        "height": 1280,
                                                                        "box": {"x1": b_x1,"y1": b_y1,"x2": b_x2,"y2": b_y2},
                                                                        "UID": b_UID,
                                                                        "id": b_id,
                                                                        "type": "Rectangle",
                                                                        "tags": [
        ...
        with open(source+'.json', 'w') as outfile:
            json.dump(inpJson, outfile)
        print("Total Boxes",Total_boxes)
        print("Total Tubes",Total_tubes)
    return 0        
        
def process(inp_data,a):
